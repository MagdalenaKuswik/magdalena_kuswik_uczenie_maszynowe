...
IQR)].index)
df = df.drop(df.loc[df['Annual_Income'] < (Q1 -
1.5 * IQR)].index)
...
sns.heatmap(df_train[CatFeatures].corr(), annot = True, linewidths = 0.1, cmap = 'Blues')
plt.title('Numerical Features Correlation')
plt.show()

least_correlation = ['Num_Credit_Card', 'Num_of_Loan']
df_train = df_train.drop(least_correlation, axis=1)

pd.DataFrame(abs(df_train.corr()['Credit_Score'].drop('Credit_Score')*100).sort_values(ascending=False)).plot.bar(figsize = (10,8))
# usuń 2 najmniej skorelowane cechy
# I zapisz jako zbiór nr 1
plt.show()
